...
export default function UpdatePotion(props){
    console.log(props)
    const id=props.id
    ...
    const handleSubmit = async (e) => {
		...
		try {
            console.log(body)
			const url = `http://localhost:3000/update-potions/${id}`
			const { data: res } = await axios.post(url, body);
            console.log(res)
            openAlert("Pocion actualizada")
		} catch (error) {
			if (
				error.response &&
